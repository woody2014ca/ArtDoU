...
| 页面 | 路径 | 功能 |
|------|------|------|
| 首页 | index | 身份分流(审核员/老师/家长)、数据看板、学员名录、家长入口(意向缴费/绑定) |
| 审核记录 | auditLog | 审核记录列表 |
| 审核编辑 | auditEdit | 审核编辑 |
...
- **审核/未登录**：无 token 或 token 无效视为 guest，行为与小程序审核模式一致（仅演示数据或受限）。
...
- 未登录或未绑定：视为 guest，首页为审核/演示模式（本地假数据）。
